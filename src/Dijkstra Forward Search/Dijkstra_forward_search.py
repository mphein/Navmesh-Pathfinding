...
def dijkstras_shortest_path(initial_position, destination, graph, adj):
    ...
    while queue:
        ...
        for (child, step_cost) in adj(graph, cell):
            ...
            cost_to_child = priority + transition_cost(graph, cell, child) - distance(cell, destination) # transition cost will be distance cost from one box to neighbor 
            if child not in pathcosts or cost_to_child < pathcosts[child]:
                ...
                heappush(queue, (cost_to_child + distance(child, destination), child))     # put the child on the priority queue
...
if __name__ == '__main__':
    filename, src_waypoint, dst_waypoint = 'example.txt', 'c','a'
